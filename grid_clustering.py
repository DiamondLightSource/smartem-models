...
model = EIAE(alpha = torch.Tensor([[1., 1.]]).to(device), input_dims = config['input_dim'], hidden_dims = config['hidden_dims'], lat_dim=config['lat_dim'])        
...
def train(epoch, epoch_losses):
    ...
    for bidx, samples in tqdm(enumerate(train_dataloader)):
        data, labels = Variable(samples['x1']).to(device), Variable(samples['x1']).to(device)
