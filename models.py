...
class ConvT_BN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, output_padding):
        ...
        self.layers =  nn.ModuleList()
        self.layers.append(nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride = stride, \
...
class Up_Block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, output_padding):
        ...
        self.stride = stride
        self.CBR0 =  ConvT_BN_Relu(in_channels, out_channels, kernel_size = kernel_size, stride = stride, \
                                   padding = padding, output_padding = output_padding)
...
class EIAE(nn.Module):
    def __init__(self, alpha, input_dims, hidden_dims, lat_dim=2):
        ...
        out_type = enn.FieldType(self.r2_act, self.hidden_dims[4]*4*[self.r2_act.regular_repr])
        ...
        self.enc_equi1 = nn.Sequential(
            nn.Conv2d(self.hidden_dims[4]*n_rot, self.hidden_dims[4], kernel_size = 3, padding = 1),
            nn.BatchNorm2d(self.hidden_dims[4]),
            nn.LeakyReLU())
        
        self.et_fc = nn.Linear(self.hidden_dims[4]*self.reduced_dim_m*self.reduced_dim_n, self.fc_hidden)
        self.to_et = nn.Linear(self.fc_hidden, 3)


        
        self.to_dec = nn.Linear(in_features=self.latent_dim*2, out_features=self.fc_hidden)
        self.de_fc = nn.Linear(self.fc_hidden, self.hidden_dims[-1]*self.reduced_dim_m*self.reduced_dim_n)
